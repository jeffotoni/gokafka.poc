...
func NameTextBoxPosition() (x, y int) {
	x, y = levelTextBoxPosition()
	return x, y + 4*blockHeight
}
...
func init() {
	...
	// Windows: Name
	x, y = NameTextBoxPosition()
	drawTextBox(imageWindows, "NAME", x, y, textBoxWidth())
...
func (s *GameScene) Update(state *GameState) error {
	...
	if !s.field.IsFlushAnimating() {
		...
		if state.Input.IsRotateRightJustPressed() {
			...
			if moved {
				s.score++
				go ApiProducerKafka(KeyUUid, NameUserGame, s.score)
				//println("score:", NameUserGame, " ", s.score)
			}
		}
	}
...
func (s *GameScene) Draw(r *ebiten.Image) {
	...
	// Draw Name
	x, y = NameTextBoxPosition()
	drawTextBoxContent(r, substr(NameUserGame, 0, 10), x, y, textBoxWidth())
